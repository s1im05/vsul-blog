export const adminDashboard = (w, $, u) => {
	...
	const getPreloader = () => {
		...
		loadObs = (id) => {
			...
			$.post('/adm_panel/dashboard', {
				...
				let html = $('<ul class="list-group" />');
				res.forEach(f => {
					const btn = $('<button class="btn btn-xs obsOpen pull-right" />').data('id', f.id).append('<i class="fa fa-fw fa-check" />'),
						li = $('<li class="list-group-item" />').text(`"${f.title}" - ${f.text}`).append(btn);
					html.append(li);
				});
				field.html(html);
			});	
			...
			$.post('/adm_panel/dashboard', {
				...
				res.forEach(f => {
					const a = $(`<a target="_blank" href="/a/${f.article_name}#comment_${f.id}" />`).text('#'),
						...
						btn = $('<button class="btn btn-xs toggleCommentDelete pull-right" />').data('id', f.id).append('<i class="fa fa-fw ' + (f.is_deleted === '0' ? 'fa-times' : 'fa-check') + '" />'),
	...
	$(function(){
		...
		loadObs('#obs').on('click', '.fa-refresh', (e) => {
			loadObs('#obs');
		}).on('click', '.obsOpen', function(e){
			$.post('/adm_panel/dashboard', {
				'obsOpen': $(this).data('id')
			}, function(res){
				loadObs('#obs');
			});
		});
